...
const domain = 'https://top-horoscope.fun';
...
export default () => {
  ...
  const tomorrowLinks = zodiacs.map((z) => `${domain}/horoscope-${z}-on-${dateTomorrow}/`);
  const afterTomorrowLinks = zodiacs.map((z) => `${domain}/horoscope-${z}-on-${dateAfterTomorrow}/`);
  const linksWithCategory = zodiacs.map((z) => `${domain}/category/${z}/`);

  return _.concat(tomorrowLinks, afterTomorrowLinks);
};
